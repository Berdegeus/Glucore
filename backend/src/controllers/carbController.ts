/**
 * HTTP translation for `/carbs`: reads the request, calls the service, maps the
 * result to a status and a body. No Prisma, no business rule.
 */

import type { Response } from 'express';
import { auditRequestContext } from '../lib/audit';
import type { AuthRequest } from '../middleware/auth';
import type { CarbService, RequestContext } from '../services/carbService';
import { sendFailure } from './httpFailure';

export interface CarbController {
...
export function createCarbController(service: CarbService): CarbController {
  return {
    async list(req, res) {
