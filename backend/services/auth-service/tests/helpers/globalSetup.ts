...
export default function setup(): void {
  ...
  if (!/glucore_auth_test|_test(\?|$)/.test(databaseUrl)) {
    throw new Error(
  ...
  try {
    ...
    throw new Error(
      `Could not migrate the test database at ${databaseUrl}.\n` +
        'Start PostgreSQL and create the database (createdb glucore_auth_test), or point\n' +
        'TEST_AUTH_DATABASE_URL at another one via services/auth-service/.env.test.\n\n' +
        detail,
    );
  }
}
