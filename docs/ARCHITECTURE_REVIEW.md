...
**Histórico — 🟡 Parcial (2026-07-07):** `JWT_SECRET` agora é obrigatório com fail-fast no boot (`backend/src/lib/env.ts:1-11`) e o CORS é restringível por `CORS_ORIGIN` (`index.ts:17-21`) — mas sem a env cai em `cors()` aberto. Rate-limit em `/auth/login`/forgot-password continua ausente.
**Solução proposta:**
