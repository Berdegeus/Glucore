...
| TCC-06 | P2: Mensagens padronizadas | 1.4 | Tasks | Verified (7 telas restantes migradas — Fix 2, T35) |
...
**Coverage (confirmada pela re-verificação independente da rodada 2, 2026-08-17 — veredito PASS, ver `validation.md`):** os 5 gaps da rodada 1 foram reconferidos um a um com evidência nova e os 2 mutantes foram reinjetados por um verificador independente — ambos morrem. 18 total · 17 verified · 1 descoped. Observações não bloqueantes (escolha de escopo, não defeito) na seção 6 de `validation.md`: 3 ocorrências de `Colors.orange` seguem fora de `AppTheme` (cláusula ampla do AC2 de P3) e os ACs de higiene fechados por grep (`SnackBar`, l10n de `UserAppBar`, cores) não têm teste estrutural de regressão.

**Coverage (após Fix round 1, 2026-08-17 — T33–T37, ver `tasks.md` Phase 6):** 18 total · 17 verified · 1 descoped (TCC-16 — target file removed by an earlier revert). Os 5 gaps do relatório de verificação independente (`validation.md`) foram fechados: limite de 8 caracteres asserido nos dois lados e os 2 mutantes reinjetados morrem (T33); campo de senha da troca de e-mail migrado para `PasswordField` (T34); as 7 telas restantes migradas para `GlucoreMessenger` — `grep -rn "SnackBar(" lib` só acha a definição em `glucore_messenger.dart` (T35); texto novo de `UserAppBar` movido para l10n reusando chaves existentes (T36); as 14 ocorrências residuais de `Colors.red`/`Colors.green` migradas para `AppTheme` — `grep -rn "Colors\.red\|Colors\.green" lib` vazio (T37).

\* **TCC-06 (item 1.4):** fechado por T35. `GlucoreMessenger` é agora o único lugar do app que constrói `SnackBar`; as 7 telas que ainda faltavam (`insulin_entry_page.dart`, `carb_entry_page.dart`, `insulin_edit_page.dart`, `carb_edit_page.dart`, `alert_settings_page.dart`, `add_observation_sheet.dart`, `libre_nfc_page.dart`) foram migradas.

† **TCC-14 (item 1.1):** fechado por T36 (texto novo de `UserAppBar` em l10n) e T37 (as 14 ocorrências residuais de `Colors.red`/`Colors.green` em `carb_edit_page.dart`, `insulin_edit_page.dart`, `sensor_link_page.dart`, `glucose_chart.dart`, `libre_nfc_page.dart` e `lib/l10n/localized_values.dart` migradas para `AppTheme.zoneLowBg`/`AppTheme.zoneTargetBg` conforme o significado semântico de cada uso).
